...
# Render's free plan does not provide an interactive service shell. Seeding is
# enabled by default so a fresh deployment gets its catalogue automatically.
# Set SEED_ON_STARTUP=false when using a paid pre-deploy seed command instead.
if os.getenv("SEED_ON_STARTUP", "true").lower() in {"1", "true", "yes", "on"}:
